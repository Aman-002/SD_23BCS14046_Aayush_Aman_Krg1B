...
@Component({
      ...
      <div class="auth-card">
        ...
        <form (ngSubmit)="onSubmit()" #f="ngForm">
          ...
          <div class="form-group">
            ...
            <input type="password" [(ngModel)]="password" name="password" placeholder="Your password" required minlength="8" #passwordRef="ngModel" />
            <div class="validation-error" *ngIf="passwordRef.invalid && (passwordRef.dirty || passwordRef.touched)">
              ...
              <small *ngIf="passwordRef.errors?.['minlength']">Password must be at least 8 characters.</small>
            </div>
          </div>
        ...
        <div class="demo-box">
          <a routerLink="/admin-login" style="color: #FF8C00; font-weight: bold; text-decoration: none;">Are you an Administrator? Login Here</a>
        </div>
      </div>
    </div>
  `,
  styles: [`
    .auth-wrapper { min-height:100vh; display:flex; align-items:center;
      ...
      background: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('/assets/universal.jpg');
    ...
    .auth-footer a { color:#FF8C00; font-weight:600; text-decoration:none; margin:0 4px; }
    .demo-box { margin-top:16px; background:#f1f8e9; border-radius:8px;
      padding:12px; font-size:12px; color:#33691e; text-align:center; line-height:1.6; }
  `]
})
      ...
      next: (res: any) => {
        ...
        if (res.success) {
          if (res.data.userType === 'Admin') {
            this.error = 'Administrators must log in via the Admin Portal.';
            return;
          }
